...
executable = '/home/fhong5/crowderDNA_new/crowderoxdna/build/bin/oxDNA'
input = 'input'
logfilename = 'ffs.log'
...
condition_file.write("condition1 = {\n%s < %s\n}\n" % (lambda_0_name, str(lambda_0_value)))
...
try:
	...
	base_command = []
base_command += [executable, input, 'print_energy_every=1e5', 'print_conf_every=1e6','no_stdout_energy=1','refresh_velocity=0','restart_step_counter=0']
...
def f(lidx):
	...
	my_base_command = base_command + ['conf_file=%s' % (my_conf), 'lastconf_file=%s' % (my_conf), 'seed=%d' % (seed)]

	# edit the command; we set to 0 the timer ONLY for the first time
	command = my_base_command + ['ffs_file=apart-bw.txt', 'restart_step_counter=1']
	...
	while success_count.value < desired_success_count:
		...
		output.seek (0)
		command = my_base_command + ['ffs_file=apart-fw.txt']
		r = sp.call (command, stdout=output, stderr=sp.STDOUT)
		assert (r == 0)
		...
		output.seek(0)
		command = my_base_command + ['ffs_file=both.txt']
		r = sp.call(command, stdout=output, stderr=sp.STDOUT)
		assert (r == 0)
		...
		success = eval ('op_values["%s"] %s %s' % (lambda_0_name, '<', str(lambda_0_value)))

...
def calculating_crowder_number(box_size = 20.0, exlude_volume_fraction = 0.3, crowder_radius = 1.0):
    ...
    print(int(crowder_number),exlude_volume_fraction,int(crowder_number)*crowder_volume/box_volume)
...
mass_set = [1]
diffusion_set = [1.25,2.5,5,7.5]

box_size = 20

for i in range(0, len(mass_set)):
    crowder_mass = mass_set[i]

    for j in range(0, len(diffusion_set)):
        diff_coeff_crowder = diffusion_set[j]

        crowder_num = calculating_crowder_number(box_size, exlude_volume, crowder_radius)

        folder_name = 'crowder_'+'mass'+str(crowder_mass)+'_diff'+str(diff_coeff_crowder)
